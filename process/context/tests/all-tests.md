...
- `backend/tests/crud.test.js` covers source lifecycle, Apify source presets and bootstrap, signal metadata/timeline, alert list/detail/status/assignment, alert summary type distribution/timeline, workspace settings, workspace members including registered-user email invites, case lifecycle, and integration lifecycle with workspace-scoped mocks.
...
- `backend/tests/workers.test.js` mocks `bullmq.Worker` to capture processors directly; it covers ingestion worker Apify processing, cancellation/retry/final-failure, AI analysis persistence/fallback failure logs, alert detection/escalation dispatch with partial workspace failure recovery, and notification event dispatch/unknown-event failure.
