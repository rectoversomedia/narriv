...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupFormValues) => {
    ...
    try {
      ...
      if (res.status === 409) {
        setApiError(t("errors.emailRegistered"));
        return;
      }
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 flex flex-1 flex-col p-12">
          <Image src="/narriv-logo-light.png" alt="Narriv" width={160} height={40} priority style={{ width: 160, height: 40 }} />
          ...
          <div className="my-auto max-w-[500px]">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#465FFF33] bg-[#465FFF14] px-4 py-2">
              ...
              <span className="text-[12px] font-semibold uppercase tracking-[0.1em] text-[#6B7FFF]">{t("earlyAccess")}</span>
            </div>

            <StreamingHeadline text={shared("signupHeadline")} />

            <p className="mt-5 text-[16px] leading-[1.6] text-[#64748B]">
              {shared("signupDescription")}
            </p>
      ...
      >
        <div className="mb-8 lg:hidden">
          <Image src="/narriv-logo-light.png" alt="Narriv" width={140} height={36} priority style={{ width: 140, height: 36 }} />
        </div>
        ...
        <div className="w-full max-w-[424px] rounded-[28px] border border-[#E2E8F0] bg-white/95 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur sm:p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            <div className="space-y-3">
              ...
              <div>
                ...
                />
                <PasswordStrength password={password} labels={strengthLabels} suffix={t("strength.password")} />
              </div>
            </div>
            ...
            <p className="mt-4 text-[12px] leading-normal text-[#334155]">
              {t("termsPrefix")}{" "}
              <button type="button" className="cursor-pointer text-[#465FFF] hover:underline">{t("terms")}</button>{" "}
              {t("termsMiddle")}{" "}
              <button type="button" className="cursor-pointer text-[#465FFF] hover:underline">{t("privacy")}</button>.
            </p>
            ...
            >
              {isSubmitting ? <><Loader2 className="animate-spin" size={17} /> {t("submitting")}</> : t("submit")}
            </button>
          </form>
          ...
          >
            ...
            <p className="text-[12px] text-[#475569]">{t("earlyAccessFree")}</p>
          </div>

          <p className="mt-6 text-center text-[13px] text-[#334155]">
            {t("alreadyHaveAccess")}{" "}
            <Link href="/login" className="cursor-pointer font-semibold text-[#465FFF] hover:underline">{t("signinLink")}</Link>
          </p>
        </div>
      </div>
    </main>
