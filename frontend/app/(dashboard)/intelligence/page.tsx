    ...
    <div className="space-y-6 pb-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_382px]">
        <DesignFrame className="flex min-h-[600px] flex-col gap-5">
          ...
          <p className="theme-muted text-[13px]">{t.intelligence.mapDesc}</p>
        ...
        <DesignFrame className="flex min-h-[600px] flex-col gap-4">
          ...
          <p className="theme-muted text-xs font-medium uppercase tracking-[0.06em]">{t.intelligence.source}</p>
          <h3 className="theme-text mt-2 text-[22px] font-semibold leading-[1.15]">{t.intelligence.clusterTitle}</h3>
          <InnerPanel className="mt-2 min-h-28 rounded-xl p-4"><p className="theme-soft text-[13px] leading-[1.45]">{t.intelligence.clusterDesc}</p></InnerPanel>
          <p className="theme-muted text-xs font-medium uppercase tracking-[0.08em]">{t.intelligence.metrics}</p>
          <div className="mt-2 grid grid-cols-2 gap-3">
            <InnerPanel className="min-h-[88px] rounded-xl p-4"><p className="theme-muted text-xs">{t.intelligence.velocity}</p><p className="mt-3 text-base font-semibold text-[#F97066]">+38%</p></InnerPanel>
            <InnerPanel className="min-h-[88px] rounded-xl p-4"><p className="theme-muted text-xs">{t.signals.confidence}</p><p className="theme-text mt-3 text-base font-semibold">94%</p></InnerPanel>
          </div>
          <InnerPanel className="mt-2 min-h-[100px] rounded-xl p-4"><p className="theme-text text-[15px] font-semibold">{t.intelligence.focus}</p><p className="theme-soft mt-2 text-[13px] leading-[1.4]">{t.intelligence.focusDesc}</p></InnerPanel>
          <button className="mt-auto h-11 w-full rounded-lg bg-[#465FFF] text-sm font-medium text-white transition-opacity hover:opacity-90" type="button">{t.intelligence.open}</button>
        </DesignFrame>
      </div>
    </div>
  );
