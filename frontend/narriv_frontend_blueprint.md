...
| Widget | Detail | Data Source |
|--------|--------|-------------|
| AI Summary + KPIs | Executive summary, 24h signal metrics, and alert-backed critical count | `getSignalsMeta().metrics` and `getSignalsMeta().aiSummary`; no synthetic sparkline trends |
| Follow-up Panel | Recommended next response and ownership context | `getSignalsMeta().followUps`; empty/error state when API data is unavailable |
| Filter Row | Sentiment filters and search — fully bilingual (EN/ID) | Server-side `sentiment` query via `getSignals()` plus keyword search; sort is shown as a non-interactive "Latest first" status because the backend orders by newest captured signal |
| Signals Table | Main table with signal, source, sentiment, published/captured time, and real Investigate action | Wired to `getSignals()` with search, 24h params, sentiment filter, pagination, and no sample fallback rows; removed frontend-synthesized severity, trend, velocity, mentions, and confidence columns |
| Recommendations | AI action suggestions beside the table | `getSignalsMeta().recommendations`; empty/error state when API data is unavailable |
...
| Create Investigation Modal | Modal form for creating cases — fully bilingual | Wired to `createCase()` API; table row Investigate pre-fills `signalId` and signal title |
...
| Bilingual Coverage | All UI text (titles, descriptions, filters, headers, footers, error states, empty states) translated to EN/ID | `next-intl` via `useTranslations("Signals.*")`; sample fallback copy removed for production accuracy |

#### 🔔 Alerts (`/alerts`)
| Widget | Detail | Data Source |
|--------|--------|-------------|
| Metric Cards | Alert volume, severity, lifecycle coverage, escalation, overdue, and response-time KPIs | `getAlertsSummary()` total/severity/status counts, acknowledged/resolved/escalated/overdue counts, lifecycle coverage rate, SLA target, and live timeline sparklines; cards show loading skeletons and explicit error state instead of stale zeroes |
...
| Alert Table | Main alert list with severity, owner, source, escalation, deadline, acknowledgment, status, and time | Wired to `getAlerts()` with server-side search, filters, real pagination, loading/error/empty states, and no mock fallback rows |
...
| Notification Rules | Header modal for workspace delivery toggles, presets, and custom notification rules | `getNotificationSettings()` + `updateNotificationSettings()` against `/api/workspace/notification-settings`; persists email/instant/escalation/reminder toggles plus custom trigger/condition/channel/enabled rules; bilingual loading/error/validation states |
...
| Critical Incident | Highlighted live critical incident card with detail navigation | `getAlerts({ severity: "critical", limit: 100 })`; shows only API-backed fields (severity, owner/team, escalation, deadline, acknowledgment, status/source) with loading/empty states; removed static sentiment/confidence/mentions placeholders |
| Stakeholder Engagement | Workspace recipients plus acknowledged, resolved, and escalated alert lifecycle counts | `getWorkspaceMembers()` + `getAlertsSummary()`; no static engagement counts; loading state reflects both member and summary queries |
| Critical Delivery Status | Live critical alert status card with View All modal | `getAlerts({ severity: "critical", limit: 100 })`; no static counts/timestamps; modal lists critical alerts with owner/status/time and links to details |
| Delivery Status | Active notification/delivery channels from workspace integrations | `getIntegrations()`; displays active integrations with platform, sync status, loading/empty states; "View all channels" links to `/workspace/integrations` |
| Alert Journey | Live lifecycle timeline for selected/top alert | Derived from alert created time, owner/team, escalation level, acknowledgment/resolution status, and deadline; "View Timeline" opens a modal with the full live timeline, no static timestamps |
| Alert Detail | Drill-down page at `/alerts/[id]` | Wired to `getAlertById()` with polished live hero, status actions, context panels, recommendation CTA, and production empty/not-found handling |
| Status Actions | `updateAlertStatus()` and `updateAlertAssignment()` wired | Status dropdown menu on alert list; detail page assignment uses workspace member and escalation role dropdowns to prevent typo-prone free text |
...
| AI Search Sandbox | Search input → real API call `POST /api/visibility/analyze`; loading spinner; bilingual response parsing | `triggerVisibilityAnalysis()` uses `getWorkspaceSettings().brandName` for tenant-correct brand analysis, blocks simulation until a workspace brand exists, invalidates visibility queries after success, and shows honest analysis metadata instead of fake citation links |
...
| Error Handling | List, summary, and trend query failures show an explicit retry state | `getVisibility()`, `getVisibilitySummary()`, and `getVisibilityTrends()` all participate in page-level error handling; no sample fallback copy |
...
| Lifecycle Metrics | Emerging / Accelerating / Peaking / Declining / Dormant counts and trends | Live-derived from `getNarratives()` cluster velocity, impact, and signal counts; empty phases render zero-state sparklines instead of synthetic activity |
...
| Footer Metrics | Data Coverage, Update Frequency, AI Model Status | Wired to `getSources()` to calculate active vs total source coverage and reflect real-time API health with bilingual degraded-state copy |
...
| `getSignals()` / `getSignalsMeta()` | `GET /signals`, `GET /signals/meta` | Yes | Yes — `/signals` sends page, limit, keyword, platform, date range, and sentiment query params; table renders only API-backed signal/source/sentiment/time fields and row Investigate opens case creation. `/signals/meta` maps summary metrics, follow-ups, recommendations, source donut, timeline labels/peak, and investigation queue |
| `getAlerts()` / `getAlertById()` | `GET /api/alerts`, `GET /api/alerts/:id` | Yes | Yes — list page and detail page use React Query; list supports server-side search, severity/status filters, capped pagination, and critical-alert secondary queries |
| Alert status/assignment functions | `PATCH /api/alerts/:id/status`, `PATCH /api/alerts/:id/assign` | Yes | Yes — alerts page dropdown for status change; alert detail page assignment uses registered workspace member and escalation role dropdowns |
...
| `triggerVisibilityAnalysis()` | `POST /api/visibility/analyze` | Yes | Yes — AI Search Sandbox calls this for real-time query simulation with bilingual response parsing, requires workspace brand name, invalidates visibility queries after success, and does not render fake citation links |
...
| `getNotificationSettings()` / `updateNotificationSettings()` | `GET/PATCH /api/workspace/notification-settings` | Yes | Yes — settings page notification toggles and Alerts page Notification Rules modal are wired, including `customRules` trigger/condition/channel/enabled rows |
...
- [x] **Signals Page** — Wired the main table to `getSignals()` with API field mapping, search, 24h/7d/30d params, pagination, loading/error/empty states, and preview fallback. Expanded `getSignalsMeta()` usage for live sidebar panels, source donut total, timeline labels, and peak tooltip; backend `/signals/meta` timeline now uses 24h Signal data. Completed 2026-05-31; expanded 2026-06-06. Fully bilingual 2026-06-13: all panels (SummaryPanel, FollowUpPanel, RecommendationPanel, SourceDonut, TimelineChart, InvestigationQueue), filter tabs, time range options, table headers, search placeholder, footer text, and error states translated via `next-intl`. Mock fallback functions (getSignalRows, getFollowUps, getRecommendations) removed 2026-06-13. Production-ready pass 2026-06-28 removed misleading frontend-synthesized severity/trend/velocity/mentions/confidence columns, removed sample fallback copy, wired server-side sentiment filtering, surfaced metadata API errors, and made row Investigate open the case creation modal with the selected signal.
- [x] **Alerts Page** — Main list wired to `getAlerts()` with pagination. Status/assignment mutations wired via dropdown menu and editable assignment fields on detail page. Expanded `getAlertsSummary()` usage for live severity/status metric cards, source distribution donut, and 24h timeline/peak tooltip on 2026-06-06. Production-readiness pass added server-side search, capped pagination, lifecycle metric cards, critical incident/status panels, delivery channel integration data, stakeholder engagement, Alert Journey modal, editable escalation matrix, and Notification Rules modal backed by workspace notification settings. Completed 2026-05-31; expanded 2026-06-06; production-ready panels expanded 2026-06-28.
- [x] **Alert Detail Page** — Replaced mock lookup with `getAlertById()` and React Query state handling. Mock fallbacks removed 2026-06-05. Detail page now has live hero/status actions, context panels, recommendation CTA, and assignment dropdowns sourced from workspace members plus escalation matrix roles. Completed 2026-05-30; production UI pass 2026-06-28.
- [x] **Visibility Page** — Complete production-ready overhaul. Removed all mock/fallback data. 100% API-driven: `getVisibility()`, `getVisibilitySummary()`, `getVisibilityTrends()`. Features: date filter (7/14/30/90 days) with quick toggle; CSV export; dynamic executive summary from trend deltas; AI Search Sandbox with real `triggerVisibilityAnalysis()` API call and bilingual response parsing; SVG line charts (Mentions Trend, Competitor vs); donut chart (Share of Voice); Top Platforms bar chart; Top Topics table with link to Intelligence page; Latest Mentions with "View all" modal; AI Actions with "See all" modal; all empty states; all i18n (EN/ID) with 60+ translation keys; bilingual AI responses (EN/ID). Completed 2026-05-31; expanded 2026-06-06; production-ready 2026-06-21. Production-hardening pass 2026-06-28 added explicit error handling across visibility/summary/trends queries, removed stale sample-data copy, made Sandbox require workspace brand name instead of using hardcoded fallback, removed fake citation links, added analysis metadata, and invalidates visibility queries after a successful analysis.
- [x] **Intelligence Page** — Wired to `getNarratives()` with `buildNarrativeClusters` mapping. Competitor narrative share donut and lifecycle cards are live-derived from cluster signal counts, velocity, and impact. AI Summary is generated dynamically from live cluster data. Footer metrics wired to `getSources()`. Interactive panels (Analysis, Landscape, All Clusters) wired to React Portal modals. Completed 2026-05-31; mock fallbacks removed 2026-06-05; secondary panels and dynamic interactions expanded 2026-06-19. Production-ready pass 2026-06-28 removed synthetic lifecycle activity for empty phases, added query error handling across list/detail/source reads, localized remaining hardcoded UI/toast copy, disabled narrative actions when no live narrative is selected, and made selected narrative badges reflect live impact/lifecycle data.
...
- [x] **Remove Mock Data** — All primary and secondary dashboard widgets (topics, sources, system status, signals sidebar panels) now use live API endpoints (`getDashboardSummary` and `getSignalsMeta`). Signals no longer renders sample fallback rows or sample fallback copy when the API fails.
